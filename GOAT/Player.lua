...
function Player:init(map)
    ...
    self.map = map
    self.texture = love.graphics.newImage('graphics/wk6kC6F.png')

    -- sound effects
    self.sounds = {
        ['jump'] = love.audio.newSource('sounds/GoatBah.mp3', 'static')
    }
    ...
    self.animations = {
        ['idle'] = Animation({
            ...
            frames = {
                love.graphics.newQuad(48, 105, 44, 40, self.texture:getDimensions())
            }
        }),
        ['walking'] = Animation({
            texture = self.texture,
            frames = {
                love.graphics.newQuad(4, 105, 40, 40, self.texture:getDimensions()),
                love.graphics.newQuad(100, 105, 40, 40, self.texture:getDimensions()),
                love.graphics.newQuad(4, 105, 40, 40, self.texture:getDimensions()),
                love.graphics.newQuad(100, 105, 40, 40, self.texture:getDimensions()),
            },
            interval = 0.15
    ...
    self.behaviors = {
        ...
        ['jumping'] = function(dt)
            ...
            elseif self.map:fcollides(self.map:tileAt(self.x, self.y + self.height)) or
            self.map:fcollides(self.map:tileAt(self.x + self.width - 1, self.y + self.height)) then
